<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <title>Tema 4 Mostrar Carga Inicial  David Aparicio</title>
        <link rel="stylesheet" href="../webroot/css/estilos.css"/>
    </head>
    </head>
    <body>
        <header>
            <h1>Tema 4 Mostrar Carga Inicial Base de Datos David Aparicio </h1>
            <div id="nav">
                <h2> Carga Inicial de la base de datos </h2>
            </div>
        </header>
        <?php
        highlight_file("../scriptDB/CargaInicialDAW201DBProyectoTema5.sql");
        ?>
        <footer> 
            <a href="../../doc/CVDavidAparicioSir.pdf" target="blank"><img src="../doc/img/cv.png" alt="CV David Aparicio"/></a>
            <a href="../indexProyectoTema5.php"><img src="../doc/img/home.png" alt="HOME"/></a>
            <a href="https://www.github.com/davidas02" target="_blank"><img src="../doc/img/git.png" alt="github David Aparicio"/></a>
            <p>2022-2023 David Aparicio Sir &COPY; Todos los derechos reservados</p>
        </footer>
    </body>
</html>
